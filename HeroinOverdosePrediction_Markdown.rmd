---
title: "Predicting Heroin Overdose Events in Cincinnati, Ohio"
...
The Ohio Valley, situated in midwestern, midsouthern United States, is embroiled in a public health crisis. Opioid abuse is widespread across Ohio, Pennsylvania, Kentucky and Indiana[^1][^2][^3]. This epidemic of overdose deaths has driven a 29% increase in the mortality rate of Americans aged 29-34 nationwide since 2010[^4]. Ohio reports the second-highest rate of opioid-related overdose deaths in the country[^5], with fatalities most clustered in the state's southwestern counties.  In these counties, overdose death rates are two to four times the national average[^5][^6]. Preventing and decreasing deaths due to overdose is an urgent policy priority in the region.

The following analysis focuses on southwestern Ohio's largest city: Cincinnati. Cincinnati declared heroin overdoses an "epidemic" in 2016 after 174 heroin-related deaths occurred within the span of a single week.  These deaths overwhelmed the County Medical Examiner's office and strained first responders' ability to administer Narcan (naloxone), an opioid overdose reversal drug, at the scene of suspected overdose events[^7]. To combat this epidemic, the city focused on its EMS deployment, leveraging dispatcher call data and ambulance response data to map heroin overdoses, identify overdose hotspots, and more effectively target its medical resources and personnel towards overdose response and prevention across the city[^8]. 

The data used to inform Cincinnati's EMS decision-making are available to the public [here](https://insights.cincinnati-oh.gov/stories/s/Heroin-Overdose-Responses/dm3s-ep3u/). 
...
Medical personnel are the most prominent and immediate source of treatment for heroin overdoses. However, non-medical personnel play a large role in overdose prevention. In 2017, the Hamilton County Department of Public Health founded the Narcan Distribution Collaborative (NDC) to distribute free take-home naloxone kits for the public and to host naloxone training workshops for their usage[^9][^10]. 

The NDC quickly outstripped Emergency Medical Services (EMS) and pharmacies as the largest consumer and distributor of naloxone in the region[^11]. The organization has distributed more than 20,000 doses of naloxone since 2017[^9], primarily to individuals or organizations who seek out the NDC's events or request a workshop. As seen below, NDC distribution sites are not entirely located near overdose clusters. Improving the targeting of the NDC's marketing and kit distribution could place naloxone into the hands of the right people, saving more lives.
...
We propose that overdose deaths could be further reduced if the NDC had access to a hyperlocal spatial risk assessment and used it to select seasonal sites for kit distribution and training workshops.

Two recent spatial analyses of Cincinnati's EMS data[^13][^14]suggest that heroin misuse and overdose are hyperlocal phenomena correlated with residents' age, sex and socioeconomic status; as well as features of the city's built environment, such as likely locations of drug sale (abandoned properties, abandoned cars, neighborhoods zoned for manufacturing or parks) and retail common to low-income, commercial, or industrial neighborhoods (fast food restaurants, pool halls, adult entertainment venues); and several types of crime (assault and violent crime). The following analysis builds on this literature to pursue the following goal: predict seasonal heroin overdose hotspots in Cincinnati to help the NDC proactively distribute naloxone kits and train their users in high-risk locations. 
...
Data for this analysis were gathered from the following sources. Information spans the years 2015 - 2019 unless otherwise noted.
  ...
  - **[Google Maps](www.google.com/maps), [QSR Magazine](https://www.qsrmagazine.com/content/32-biggest-fast-food-chains-america), [Wikipedia](https://en.wikipedia.org/wiki/List_of_fast_food_restaurant_chains), [Thrillist](https://www.thrillist.com/eat/nation/best-midwest-restaurant-chains-fast-food)**
      - Fast food chain restaurant locations. Li et al. (2019)[^13] found that fast food locations were a strong covariate in predicting opioid overdose in Ohio, but as their data source proved unavailable, we compiled a list of restaurant locations based on major chains in the United States, midwest, and Ohio, and generated a KML file manually through Google Maps.
...
This analysis focuses on heroin overdoses, rather than the wider category of opioid overdoses. Literature from the Hamilton County Department of Public Health, Ohio Department of Health, National Institute on Drug Abuse, and Centers for Disease Control report that the number of heroin overdose deaths in Ohio increased sharply between 2010 and 2016, specifically reporting a statistically significant one-year increase in Ohio from 2016 to 2017[^15], and have been in decline since 2017[^16]. (Drug overdose deaths caused by the synthetic opioid fentanyl have caused an increase in _all_ opioid-related overdose deaths during this same time period, but fentanyl falls outside the scope of our analysis)[^17]. 
...
her2019 <- ggplot() +
  geom_sf(data = cincinnati, fill = NA) +
  geom_sf(data = heroin_ems19, size = 1, color = "#25CB10") +
  labs(title="2019") +
  theme(legend.position = "none") +
  mapTheme()
...
The literature on heroin use indicates that several features of a city, neighborhood, or street corner's built environment, socioeconomic profile, retail landscape, visible blight, and violent crime rates are associated with an increased risk of heroin overdose events. Below is the spatial distribution of these risk factors in Cincinnati. First are maps of factors that exist as time-invariant points in space.
...
# Risk factor plots with grid.arrange instead
expl_vac <- ggplot() +
  geom_sf(data=cincinnati) +
...
The maps suggest that most of these factors demonstrate spatial clustering similar to Cincinnati's heroin overdose sites and may therefore be useful in predicting overdose risk.

The features that do not appear to mimic the spatial process are those derived from 311 non-criminal complaints about trash; crimes classified as murder/homicide; and the locations of firearms dealers. Trash complaints and assaults are too prevalent to be of use, while firearms and weapons retail are too sparse.
...
Heroin use in Ohio is most prevalent among White, non-Hispanic males aged 25-44[^16], and it does appear that the proportion of young- and middle-aged White men in Cincinnati tracts demonstrates a roughly similar distribution to overdose events. Poverty and low rates of college education also appear to be correlated with overdose events.

# Spatial-time process 

As seen in the data, the temporal process is similar to a negative quadratic function (a parabola that opens downard), while the spatial process displays clustering, most heavily in an arc across the southern side of the city, straddling train tracks and a waterway in south-central Cincinnati. 

### Units of spatial analysis

To better understand spatial interrelationships between overdoses and related factors, we overlay a grid on the city (referred to as the "fishnet") with 1000-square-foot cells. Using grid cells as a unit of analysis allows us to aggregate factors at hyperlocal geographies. In a sense, the size of the grid cell (1000 sq. ft) is arbitrary; we chose a size large enough to demonstrate meaningful variation from other cells but small enough to capture phenomena that might be local to such subsections of the city. 
...
ggplot() +
  ...
  scale_fill_viridis() +
  labs(title = "Number of heroin overdose responses per fishnet cell, 2015-2018", fill = "EMS response\ncount") +
  mapTheme()
```
...
#### Count of risk factor events
First, we take a straightforward approach: counting the number of factor incidences per grid cell.  For example, the number of violent crimes per grid cell. While aggregation by grid cell is a fairly intuitive approach, it is prone to bias because the summary values are influenced by the arbitrary scale of each cell[^18].

Note that aggregation only applies to data that are points in space (e.g., locations of vacant buildings, assaults, fast food restaurants) and does not apply to polygon data that are spread across larger scales (e.g., a tract of land zoned for manufacturing use). We exclude the location of firearms dealers as a factor because there are so few data.
...
ab <- ggplot() +
  ...
  mapTheme()
ac <- ggplot() +
  geom_sf(data = filter(vars_net.long, Variable == "Assault Crimes"), aes(fill=value), colour=NA) +
  scale_fill_viridis(name="") +
  labs(title="Assault Crimes", fill = "Count") +
  mapTheme()
ae <- ggplot() +
...
grid.arrange(ab, ac, ncol = 2)
grid.arrange(ae, ff, ncol = 2)
grid.arrange(pc, vb, ncol = 2)


```

#### Nearest-neighbor distance to risk factor events

Next, we re-engineer each risk factor point feature as an average nearest-neighbor distance variable. In other words, we calculate the average distance from the center of a grid cell to the three or four nearest points of the risk factor. This provides more flexibility than the aggregation approach and is potentially a bias-reduced estimate of risk exposure.
...
vars_net$assault.nn =
  nn_function(st_coordinates(st_centroid(vars_net)), st_coordinates(crime_assault), 4)
...
ab.nn <- ggplot() +
  geom_sf(data = filter(vars_net.long.nn, Variable == "abandoned_cars.nn"), aes(fill=value), colour=NA) +
  scale_fill_viridis(name="") +
  labs(title="Abandoned Cars.nn", fill = "Count") +
  mapTheme()
ac.nn <- ggplot() +
  geom_sf(data = filter(vars_net.long.nn, Variable == "assault.nn"), aes(fill=value), colour=NA) +
  scale_fill_viridis(name="") +
  labs(title="Assault Crimes.nn", fill = "Count") +
  mapTheme()
ae.nn <- ggplot() +
...
grid.arrange(ab.nn, ac.nn, ncol = 2)
grid.arrange(ae.nn, ff.nn, ncol = 2)
grid.arrange(pc.nn, vb.nn, ncol = 2)

```

#### Distance to Union Terminal

Next, we explore the distance to a single point of interest, the Union Terminal train station in Cincinnati. 

The heroin overdoses on the south side of the city are heavily clustered but report a vertical blank white space in the center of the main cluster; these are railroad tracks, a small waterway and Interstate 75, which run north-south and terminate at the Ohio River that marks the southern boundary of Cincinnati. This transit hub is centered around Union Terminal, a train station and transit center, and each grid cell's Euclidean distance to Union Terminal is calculated as a possible risk factor for overdose. 

```{r unionterminal_data}
###############################################################################
##  Euclidean distance
###############################################################################
UTpoint <-
...
Finally, we explore polygon data (e.g., neighborhoods) as possible covariates for heroin overdoses. TODO: Add more here.
...
final_net_test <- 
  left_join(heroin_net_test, st_set_geometry(vars_net, NULL), by="uniqueID")

final_net_backup <- final_net
final_net_test_backup <- final_net_test

# Add in count of manufacturing/recreational land use zones that intersect with grid cell
zones <- zones_likely_drug_sale %>% dplyr::select() %>%
  st_transform(st_crs(fishnet)) %>%
                         mutate(Legend = "Manufacturing or Rec Zoning") %>%
  st_join(., fishnet, join=st_within) %>%
  st_set_geometry(NULL) %>%
  group_by(uniqueID, Legend) %>%
  summarize(count = n()) %>%
...
final_net <-
  left_join(final_net, st_set_geometry(zones, NULL), by="uniqueID") 

final_net_test <-
  left_join(final_net_test, st_set_geometry(zones, NULL), by="uniqueID")

# Join ACS/Census data to grid cells
# This creates duplicates because a single grid cell intersects with more than one tract
acsData_props_pov <- acsData_props %>% filter(variable == "prop_poverty")
acsData_props_bach <- acsData_props %>% filter(variable == "prop_bach_or_higher")
acsData_props_male <- acsData_props %>% filter(variable == "prop_males_25to44")

final_net_TEMP <-
  st_centroid(final_net) %>%
  st_join(., dplyr::select(acsData_props_pov %>% dplyr::mutate(prop_poverty = estimate), prop_poverty)) %>%
  st_join(., dplyr::select(acsData_props_bach %>% dplyr::mutate(prop_bach_or_higher = estimate), prop_bach_or_higher)) %>%
  st_join(., dplyr::select(acsData_props_male %>% dplyr::mutate(prop_males_25to44 = estimate), prop_males_25to44)) %>%
...
# Create grid-cell-level averages of ACS/Census data 
library(plyr)
final_net_TEMP <- plyr::ddply(final_net_TEMP,"uniqueID", transform, prop_pov=mean(prop_poverty)) %>% 
  dplyr::select(-prop_poverty)

final_net_TEMP <- plyr::ddply(final_net_TEMP,"uniqueID", transform, prop_college=mean(prop_bach_or_higher)) %>% dplyr::select(-prop_bach_or_higher) 

final_net_TEMP <- plyr::ddply(final_net_TEMP,"uniqueID", transform, prop_young_males=mean(prop_males_25to44)) %>% dplyr::select(-prop_males_25to44)

final_net_TEMP <- plyr::ddply(final_net_TEMP,"uniqueID", transform, prop_wh=mean(prop_white)) %>% dplyr::select(-prop_white) 

vars <- c(2, 19:22)
final_net_TEMP <- distinct(final_net_TEMP) %>% dplyr::select(vars)

final_net <- left_join(final_net, final_net_TEMP)
final_net_test <- left_join(final_net_test, final_net_TEMP)

# TODO: Find what final_net_test is and see if I need to replicate the below code for it.
final_net <-
  st_centroid(final_net) %>%
...
zone_map <- final_net %>% gather(Variable, Value, -geometry) %>%
  filter(Variable == zoning) %>%
  ggplot() +
  geom_sf(aes(fill = Value)) +
  scale_fill_viridis(discrete = TRUE) +
...
# TODO: Stop here and see how the maps above generate.
...
```{r correlations}
# Grouping in separate blocks so that the correlation plots display larger
...
correlation.cor3 <-
  ...
  group_by(Variable) %>%
  summarize(correlation = cor(Value, countHeroinResponses, use = "complete.obs"))
ggplot(correlation.long3, aes(Value, countHeroinResponses)) +
  geom_point(size = 0.1) +
...
correlation.cor4 <-
  ...
  group_by(Variable) %>%
  summarize(correlation = cor(Value, countHeroinResponses, use = "complete.obs"))
ggplot(correlation.long4, aes(Value, countHeroinResponses)) +
  geom_point(size = 0.1) +
...
Our modeling approach is a geospatial latent risk model that employs Poisson regression to predict areas of high risk for heroin overdose. The model is trained on a dataset that pools data from four years of EMS heroin overdose data (2015, 2016, 2017, 2018). The model is tested on data from the final year of EMS data (2019). 

### Clustering effects 

The “Local Moran’s I” is a statistic that will identify heroin overdose events that are significantly non-random (i.e., clustered) relative to adjacent grid cells on the map of Cincinnati. The goal is to identify “hot spots” in which we strongly reject a null hypothesis that the count of heroin overdose events at a given location is randomly distributed relative to proximal neighbors.

<span style="font-size:1.2em">**Local Moran's I statistics, Heroin Responses**</span>
```{r modeling}
###############################################################################
##  Moran's I
###############################################################################
...
do.call(grid.arrange,c(varList, ncol = 2))
```

Heroin overdoses are clustered in prevalent in some grid cells, but across _all_ grid cells, an overdose event is relatively rare. The distributional shape of the outcome in the graph below suggests that Poisson regression is most appropriate to deal with predicting an event with such positive skew.
...
# Plot of distribution of heroin response counts
ggplot(final_net, aes(countHeroinResponses)) +
  geom_histogram(binwidth = 1) +
  xlab("Number of heroin overdose events") + 
  ylab("Frequency that this many overdoses occur,\nby grid cell") +
...
First, a poisson regression is specified and trained on 100 random subsets of data selected from the pooled 2015-2018 dataset.

```{r training, include=FALSE}
column_names <- colnames(final_net)
reg.vars <- c("Abandoned Cars", "Adult Entertainment", "Assault Crimes", "Fast Food Restaurants", "Pest Complaints", "Vacant, Condemned Bldgs", "prop_wh", "prop_pov", "UTdistance", "Manufacturing or Rec Zoning")
...
#   dependentVariable = "countHeroinResponses",
#   indVariables = reg.vars) %>%
#   dplyr::select(cvID = neighborhood, countHeroinResponses, Prediction, geometry)
...
grid.arrange(
  reg.summary %>%
    ...
    scale_fill_viridis() +
    labs(title = "Predicted heroin responses") +
...
### Regression results
```{r regsum, results="show"}
reg <-
      glm(countHeroinResponses ~ `Abandoned Cars` + 
            `Adult Entertainment` + 
            `Assault Crimes` +
            `Fast Food Restaurants` +
            `Pest Complaints` +
            `Vacant, Condemned Bldgs` +
            prop_wh +
            prop_pov +
            UTdistance +
            `Manufacturing or Rec Zoning`, family = "poisson", 
          data = final_net)
summary(reg)
```
...
When the model is trained on data from 2015-2018, we find that it predicts close to observed values in all deciles of heroin overdose, with a tendency to overpredict. The prediction error, on average, is approximately +/-6 overdose events, as seen in the Mean Absolute Error table below.
...
Our model-building process focused on factors that are correlated with heroin overdose per the drug abuse academic literature and popular report. Factor selection was influenced by the higher heroin overdose rate among White men than men of other racial/ethnic groups, and this likely introduced bias into the model, as seen by the much greater mean absolute error for majority-Nonwhite neighborhoods in Cincinnati (+/- 4 overdose events, on average) than for majority-White neighborhoods (close to zero, on average).

```{r generalizability1, include = FALSE}
tracts17 <- 
  get_acs(geography = "tract", variables = c("B01001_001E","B01001A_001E"), 
          year = 2017, state=39, county=061, geometry=T) %>%
  st_transform(3735)  %>% 
...
```{r generalizability2, eval = FALSE}
tracts17 <- 
  get_acs(geography = "tract", variables = c("B01001_001E","B01001A_001E"), 
          year = 2017, state=39, county=061, geometry=T) %>%
  st_transform(3735)  %>% 
...
# Mean error by neighborhood context
final_reg <- 
  reg.summary %>%
  mutate(uniqueID = rownames(.))
...
As indicated, the model appears to suffer from a focus on particular types of heroin users. Per the earlier evidence, it predicts well in lower-risk areas but is not generalizable across high-risk areas and is in fact outperformed by traditional kernel density "hotspot" methods.
...
```{r robch}
  ...
  rbind(heroin_KDE_sf, heroin_risk_sf) %>%
    ...
    scale_fill_viridis(discrete = TRUE) +
    labs(title = "Kernel density model vs. risk predictions") +
    mapTheme()

```

# Testing on data from 2019

A test of this model on data from Cincinnati's 2019 EMS calls related to heroin does not fare well, suggesting significant improvements are necessary before the model could be applied to an actual use case.

```{r test}
#Predictions into 2019
reg <-
      glm(countHeroinResponses ~ `Abandoned Cars` + 
            `Adult Entertainment` + 
            `Assault Crimes` +
...
predicted_values <- predict(reg, final_net_test)
predicted_values <- cbind(predicted_values, dplyr::select(final_net_test, uniqueID, countHeroinResponses))

data <- dplyr::select(predicted_values, Predicted = predicted_values, Observed = countHeroinResponses)
ggplot() + 
  geom_point(data=data, aes(Observed, Predicted)) +
  stat_smooth(data=data, aes(Observed, Observed), method = "lm", se = FALSE, size = 1, colour="#FA7800") + 
  stat_smooth(data=data, aes(Observed, Predicted), method = "lm", se = FALSE, size = 1, colour="#25CB10") + 
  labs(title="Predicted heroin overdoses as a function\nof actual heroin overdoses in 2019",
       subtitle="Perfect prediction in orange; actual prediction in green") +
  theme(plot.title = element_text(size = 14,colour = "black"))
```

# Next steps for model improvement

We believe that future iterations of latent risk prediction models for heroin overdose in Cincinnati should approach the problem with panel data representing intervals of time and a longitudinal regression design that considers time as a predictor. Such a model could also incorporate time-varying factors like weather (temperature, precipitation) to better understand the seasonality of these tragedies. Our use of pooled cross-sectional data limited our ability to capture the shape of the change trajectory in heroin overdose prevalence in Cincinnati over a three-year period. However, our method did allow us to incorporate many time-invariant spatial features that we believed were critical to predicting latent overdose risk.  

# Conclusion: Improving the distribution of prevention resources in Cincinnati, OH

Despite the model's poor performance, we believe the data wrangling and research involved in this analysis is a strong first step toward understanding how geospatial machine learning can help those involved in preventing overdose deaths in Ohio. 

Our premise in this project was that Hamilton County, OH's Narcan Distribution Collaborative could benefit from the results of an analysis identifying areas at highest risk for overdose. The goal was to assist the NDC in the strategic distribution of take-home naloxone kits and provision of training programs for non-medical personnel. Our team envision an app that could recommend areas where NDC should focus its resources, such as buildings or locations where bystanders or companions trained tp use naloxone could be of service (parks, fast food restaurants, etc). The full proposal for the app appears here: https://youtu.be/BigWNOpK8-A.

While this analysis is not at a stage where it could support app development, it could reach that point through different strategies, such as how to model time. 
