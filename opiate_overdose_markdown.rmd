...
ggplot() +
  ...
  geom_sf(data = heroin_ems17, size = 1, color = "#25CB10", alpha = 0.3) +
  geom_sf(data = nalox_sites, size = 3, color = "#FA7800", alpha = 0.7) +
...
ggplot() +
  ...
  geom_sf(data = hamilton_streets, color = "gray") +
  geom_sf(data = cincinnati, fill = NA) +
  geom_sf(data = heroin_ems17, size = 1, aes(color = "Heroin overdose"), alpha = 0.3) +
  geom_sf(data = nalox_sites, size = 3, aes(color = "Naloxone distributor"), alpha = 0.7) +
  scale_fill_manual(values = c(
    "Heroin overdose" = "#25CB10",
    "Naloxone distributor" = "#FA7800"),
    name = "Site") +
    #breaks=c("Heroin overdose", "Naloxone distributor")) +
  labs(title="Supply vs. demand: Location of city heroin overdoses\nvs. county naloxone distribution sites", subtitle = "City of Cincinnati, Hamilton County, OH", caption = "Note: Data on heroin overdoses are only available for the city of Cincinnati; overdose\nevents in greater Hamilton County are unknown and therefore omitted from the map.") +
  mapTheme()
...
# TODO: Include any of the following?
# - Locations of fire stations, EMS, police stations, hospitals, pharmacies
# - temperature
# - precipitation
...
ggplot() +
  ...
  geom_sf(data = rbind(vacant_buildings %>% dplyr::select() %>%
                       ...
                       # graffiti %>% dplyr::select() %>%
                       #   st_transform((st_crs(fishnet))) %>%
                       #   mutate(Legend = "Graffiti Complaints"),
...
expl_mrlu <- ggplot() +
  geom_sf(data=cincinnati) +
  geom_sf(data=zones_likely_drug_sale %>% dplyr::select()) + labs(title = "Manufacturing/Recreational Land Use") + mapTheme()
...
grid.arrange(expl_vac, expl_av, expl_ct, expl_pc, expl_a, expl_mh, expl_mrlu, expl_ae, expl_fv, expl_ff, ncol=2)
...
The features that do not appear to mimic the spatial process are those derived from 311 non-criminal complaints about trash and pests as well as the locations of firearms dealers. Trash and pest complaints are too prevalent to be of use here, while firearms and weapons retail are too sparse.
...
ggplot() +
  ...
  scale_fill_viridis() +
  labs(title = "Number of heroin overdose responses per fishnet cell, 2015-2018") +
  mapTheme()
```
...
```{r counts}
###############################################################################
## Multiple map of risk factors (point data on Cincinnati boundary)
###############################################################################

# TODO: Update with our desired risk factors.
...
do.call(grid.arrange,c(mapList, ncol = 2, top = "Count of risk factors by grid cell"))
```
...
vars_net$trash_complaint.nn =
  nn_function(st_coordinates(st_centroid(vars_net)), st_coordinates(complainedTrash), 4)
...
# TODO: It'll just need to be added to the fishnet itself here.
...
ggplot() +
  ...
  scale_fill_viridis() +
  labs(title="Euclidean distance to Union Terminal transit hub", subtitle = "Cincinnati, OH") +
  mapTheme() 

###############################################################################
##  Adding non-point data to the fishnet.  i.e. stuff it overlaps with.
###############################################################################
...
# TODO: Add any metrics we want to here.
# TODO: Metrics added can be categorical (e.g. name of neighborhood) or numerical (e.g. proportion in poverty)
...
#   st_join(., dplyr::select(sna, SNA_NAME)) %>%
#   st_join(., dplyr::select(zoning, zone_type)) %>%
#   st_join(., dplyr::select(zones_likely_drug_sale, potential_drug_use_sites)) %>%
...
#   st_join(., dplyr::select(acsMedHome, estimate)) %>%
#   st_set_geometry(NULL) %>%
#   left_join(dplyr::select(final_net, geometry, uniqueID)) %>%
#   st_sf() %>%
#   na.omit()
...
##########FIX THIS ... MAYBE MAKE SEPARATE MAPS AND GRID ARRANGE?
# Map of the data under fishnet cells (e.g. zoning type, neighborhood name)
dplyr::select(final_net, Neighborhood = SNA_NAME, `Land Use` = zone_type, `Proportion in Poverty` = prop_poverty) %>%
...
ggplot(correlation.long, aes(Value, countHeroinResponses)) +
  geom_point(size = 0.1) +
...
reg.vars <- c("Abandoned Cars", "vacant_buildings.nn", "fun_times.nn", "SNA_NAME", "zone_type")
