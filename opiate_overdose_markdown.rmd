...
```
```{r heroin_grid}
# TODO: Update heroin_ems18 here and throughout the model code to whatever the training data is.
...
ggplot() +
  ...
  scale_fill_viridis() +
  labs(title = "Number of heroin overdose responses per fishnet cell") +
  mapTheme()
```
...
Next we re-engineer each risk factor point feature as the average nearest-neighbor distance from a grid cell to the three or four nearest points. This provides more flexibility than the aggregation approach and is potentially a bias-reduced estimate of risk exposure.
