...
cincy <- ggplot() +
  geom_sf(data=cincinnati) +
  labs(title = "Cincinnati, OH") +
  mapTheme()
cincy_net <- ggplot() +
  geom_sf(data=fishnet) +
  labs(title = "Fishnet of Cincinnati, OH") +
  mapTheme()
...
```{r modeling}
###############################################################################
## Heroin responses per grid cell
###############################################################################
...
#   scale_fill_viridis() +
#   labs(title = "Number of heroin responses per fishnet cell") +
#   mapTheme()
...
# NOTE: Each element of the rbind must have the same number of columns, so I dplyr::select() no columns, which by default turns the dataframe into one column of Point geographies. Then a column "Legend" is added w/ dplyr::mutate() that labels what the points represent.
vars_net <- 
...
# TODO: Claire, you may want to move this to the exploratory analysis section.  Up to you - it'll just need to be added to the fishnet itself here.
