...
```{r model_and_predPrices, results = "hide"}
sanFran.predictions <- filter(sanFran, holdOut == 0)
sanFran.predictions[reg$na.action,]$SalePrice.Predict <- mean(sanFran[which(sanFran$holdOut == 0),]$SalePrice)
sanFran.predictions[-reg$na.action,]$SalePrice.Predict <- reg$fitted.values
